...
class MemoryEnabledOrchestrator:
    ...
    async def execute(self, user_goal: str, use_memory: bool = True) -> str:
        log_action("Orchestrator", f"Starting execution for goal: {user_goal}")
        ...
        log_action("Orchestrator", f"Plan created with {len(steps)} steps for goal: {user_goal}")
        ...
        for i, step in enumerate(steps, 1):
            ...
            log_action("Orchestrator", f"Executing step {i}: {agent_name} -> {task}")
            
            if agent_name not in self.agents:
                print(f"Agent '{agent_name}' not found, skipping")
                log_action("Orchestrator", f"Agent '{agent_name}' not found, skipping")
                continue
            ...
            log_action(agent_name, f"Completed task: {task} | Output: {result[:100]}...")
        ...
        log_action("Orchestrator", f"Execution completed for goal: {user_goal} | Final result: {final_result[:200]}...")
    ...
    async def _build_comprehensive_memory_context(self, query: str) -> str:
        ...
        if important_memories:
            facts = [f"  • {mem.content}" for mem in important_memories]
            if facts:
        ...
        if similar_memories:
            relevant = [f"  • {mem.content}" for mem in similar_memories[:5] 
        ...
        if recent:
            recent_items = [f"  • {mem.content}" for mem in recent]
            if recent_items:
                context_parts.append("\n=== RECENT CONVERSATION ===\n" + "\n".join(recent_items))
...
Please create a plan considering the above context."""
    ...
    async def _build_agent_context(
        ...
        if self.memory:
            ...
            if task_memories:
                ...
                task_context = [f"  • {mem.content}" for mem in task_memories[:3]
        ...
        if previous_results:
            prev = [f"  • {r['agent']}: {r['output'][:200]}..." for r in previous_results[-2:]]
            if prev:
                context_parts.append("\n=== PREVIOUS STEPS ===\n" + "\n".join(prev))
    ...
    async def _save_to_memory(
        ...
        log_action("Memory", f"Saved to memory | Type: {memory_type} | Content: {content[:100]}...")
